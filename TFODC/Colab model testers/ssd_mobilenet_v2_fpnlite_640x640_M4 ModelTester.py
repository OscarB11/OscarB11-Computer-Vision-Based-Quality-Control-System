...
# Paths to model and test images
MODEL_DIR = r'C:\Users\besto\Documents\Local vscode\CSProjectcode\boo12\TFODC\Colab models\my_final_ssd_mobilenet_v2_fpnlite_640x640_M4\Tensorflow\workspace\models\my_final_ssd_mobilenet_v2_fpnlite_640x640_M4'
CHECKPOINT_PATH = r'C:\Users\besto\Documents\Local vscode\CSProjectcode\boo12\TFODC\Colab models\my_final_ssd_mobilenet_v2_fpnlite_640x640_M4\Tensorflow\workspace\models\my_final_ssd_mobilenet_v2_fpnlite_640x640_M4\ckpt-16.index'
PIPELINE_CONFIG_PATH = r'C:\Users\besto\Documents\Local vscode\CSProjectcode\boo12\TFODC\Colab models\my_final_ssd_mobilenet_v2_fpnlite_640x640_M4\Tensorflow\workspace\models\my_final_ssd_mobilenet_v2_fpnlite_640x640_M4\pipeline.config'
...
def load_model():
    """
    Load the detection model from checkpoint using the pipeline config
    """
...
def detect_fn(image, detection_model):
    """
    Detection function that processes the image and returns detections
    """
    # Preprocess
...
def process_detections(detections):
    """
    Process detections to get results in a more usable format
    """
...
def run_object_detection(image_path, detection_model, category_index, min_score_thresh=0.15, scale_percent=50, wait_key=0):
    ...
    try:
        ...
        # Print results
        print(f"Time taken for detection: {end_time - start_time:.2f} seconds")
        print(f"Detected faults:")
        if fault_counts:
...
def process_image_directory(image_dir, detection_model, category_index, min_score_thresh=0.15, scale_percent=50):
