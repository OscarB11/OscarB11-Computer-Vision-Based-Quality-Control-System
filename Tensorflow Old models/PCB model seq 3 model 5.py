...
from tensorflow.keras.layers import Activation, Add, GlobalAveragePooling2D, Dense, Dropout
...
def make_res_block(filters, input_layer):
    """Creates a ResNet block with a projection shortcut if needed."""
    ...
    # Adjust shortcut path to have the same shape as the main path, if necessary
...
# Residual blocks
x = make_res_block(64, x)
x = make_res_block(64, x)
x = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), strides=2, padding='same')(x)  # Reduce spatial dimensions

x = make_res_block(128, x)
x = make_res_block(128, x)
x = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), strides=2, padding='same')(x)  # Reduce spatial dimensions further if needed
